...
class AssetManager {
    ...
    [void]SyncComputerAsset([hashtable]$systemInfo) {
        ...
        try {
            ...
            $companyResult = $this.ApiClient.InvokeApi('GET', "/companies?search=$([uri]::EscapeDataString($this.Config.Snipe.StandardCompanyName))", $null)
            if ($companyResult -and $companyResult.rows -and $companyResult.rows.Count -gt 0) {
                $companyId = $companyResult.rows[0].id
                $script:Logger.Log('SUCCESS', "Using existing company: $($this.Config.Snipe.StandardCompanyName) (ID: $companyId)")
            }
            else {
                # Create new company
                $companyId = $this.GetOrCreateEntity('companies', $this.Config.Snipe.StandardCompanyName)
            }
...
"@
    ...
    [void]SyncMonitorAssets([array]$monitors, [string]$parentAssetName) {
        # Monitor synchronization removed per user request
        $script:Logger.Log('INFO', "Monitor synchronization disabled per user request")
    }
...
class InventoryOrchestrator {
    ...
    InventoryOrchestrator([hashtable]$config, [string]$configPath) {
        ...
        # Force test mode for demonstration
        $this.ConfigurationManager.Configuration.TestMode = $true
